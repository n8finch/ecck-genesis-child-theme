...
class Super_Custom_Post_Meta {
	...
	public function add_meta_box( $attr = array() ) {
		...
		foreach ( $attr['fields'] as $meta_key => $field ) {
			...
			elseif ( 'multiple_media' == $field['type'] ) $this->register_media();
	...
	if ( !empty(  $field['data-zp_desc'] ) )
		echo SCPT_Markup::tag( 'span', array(
				'id' => 'scpt_meta_' . $field['meta_key'],
				'class' => 'scpt-meta-description scpt-meta-text label-'), $field['data-zp_desc'] );
...
/**
	...
	/**
	 * Add multiple media field to a meta box
	 *
	 * @uses SCPT_Markup::tag
	 * @param array $field The field information
	 * @param array $post_meta The post meta from the database
	 ...
	 * @author Matthew Boynes
	 */
	public function add_multiple_media_field( $field, $post_meta, $html_attributes ) {
		$value = ( isset( $post_meta[ $field['meta_key'] ][0] ) ? $post_meta[ $field['meta_key'] ][0] : '' );
		...
		while($i < count($ids)){	
			if ( $ids[$i] ) {
				$attachment = get_post( $ids[$i] );
				if ( strpos( $attachment->post_mime_type, 'image/' ) === 0 ) {
					$preview .= sprintf( '<div class="zp_image_holder">');
					$preview .= wp_get_attachment_image( $ids[$i], 'thumbnail', false, array( 'class' => $ids[$i] ) );
					$preview .= sprintf( '<br /><a href="#" class="scpt-remove-multiple_media-thumbnail">%s</a>', __( 'Remove', 'super-cpt' ) );
					$preview .= sprintf( '</div>');
				} 
			} else {
				$preview .= '';
		...
		if ( false !== $field['label'] )
			echo SCPT_Markup::tag( 'label', array(
					'class' => 'scpt-meta-label scpt-meta-multiple_media label-' . $field['meta_key']
				), $field['label'] );
		echo SCPT_Markup::tag( 'div', array( 'class' => 'scpt-multiple_media-preview' ), $preview );
		echo SCPT_Markup::tag( 'p', '',
			SCPT_Markup::tag( 'a', array_merge( array(
			'href'  => '#',
			'class' => 'scpt-add-multiple_media',
			'style' => ''
			), $html_attributes ), sprintf( __( '%s', 'super-cpt' ),  __( 'Add Image', 'super-cpt' ) ) )
		);
		echo SCPT_Markup::tag( 'input', array(
			'type'  => 'hidden',
			'value' => $value,
			'name'  => $field['meta_key'],
			'class' => 'scpt-multiple_media-ids',
			'id'    => 'scpt_meta_' . $field['meta_key']
		) );
